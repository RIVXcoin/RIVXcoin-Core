...
// NOTE: Unlike wallet RPC (which use RIVX values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue estimatesmartfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric, optional) estimate fee-per-kilobyte (in RIVX)\n"
